...
public class GraphDemo {
            ...
            switch (menuReturnValue) {
                ...
                case 2: // Connected Components of G
                    ...
                    if (g.size() > 0) {
                        ...
                        int cmpCount = getComponents(g, cmp);
                        for (int cc = 1; cc <= cmpCount; cc++) {
                            System.out.println("*** Component # " + cc + " ***");
                            for (int p = 1; p <= g.size(); p++) {
                                if (cmp[p - 1] == cc) {
                                    System.out.println(g.retrieveVertex(new City(p)).getLabel());
                                }
                            }
                        }
                        System.out.print("--------------------------");
                        System.out.println("Number of Components: " + cmpCount);
                        // End code
                    } else
    ...
    private static double primMST(Graph<City> g, int root, int[] parent) throws GraphException {
        ...
        City src = new City(root);
        if (g.isVertex(new City(root))) {
            Node n = new Node(-1, src.getKey(), 0);
            pq.add(n);
        ...
        while (!pq.isEmpty()) {
            Node u = pq.poll();
            visited[u.id - 1] = false;
            totalWeight += dist[u.id - 1];
            for (i = 0; i < dist.length; i++) {
                if (!processed[i] && g.isEdge(new City(u.id), new City(i + 1))) {
                    // totalWeight = g.retrieveEdge(new City(u.id), new City(i + 1));
                    if (g.isEdge(new City(u.id), new City(i + 1))) {
                        totalWeight = g.retrieveEdge(new City(u.id), new City(i + 1));
                    } else {
                        totalWeight = g.retrieveEdge(new City(i + 1), new City(u.id));
                    }
                    Node temp = new Node(u.id, i + 1, totalWeight + dist[u.id - 1]);
                    pq.add(temp);
                    dist[i] = totalWeight;
                    processed[i] = true;
                    parent[i] = u.id;
                    visited[i] = true;
                } else if (visited[i] && (g.isEdge(new City(u.id), new City(i + 1)))
                        || g.isEdge(new City(i + 1), new City(u.id))) {
                    // totalWeight = g.retrieveEdge(new City(u.id), new City(i + 1));
